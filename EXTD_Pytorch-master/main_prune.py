...
target_pruning_rate = 0.2
config_list = [{
    'sparsity_per_layer': target_pruning_rate,
    'op_types': ['Conv2d'],
}, {
    'exclude': True,
    'op_names': [
...
def iterative_pruning_finetuning(model, bayesian_pruning_log_dir, num_iterations=50):
    ...
    for i in tqdm(range(0, num_iterations)):
        ...
        print(f"saving - {'./weights/G20/G20_{}.pth'.format(i)}")

        pruned_model = build_extd('train', cfg.NUM_CLASSES)
        pruned_model.load_state_dict(torch.load('./weights/G20/G20_{}.pth'.format(i)))
        model = pruned_model
        calc(model)
        print(f"\nEpoch execution time = {time.perf_counter() - start:.2f} second(s)")


    pruner = MultiLayerPruner(pruning_dict)
    pruner.prune(model)
    torch.save(model.state_dict(), f'./weights/extd_pruned/extd-{target_pruning_rate}.pt')
...
def main():
    ...
    print('Load network....')
    model_name = ''
    net.load_state_dict(torch.load(model_name)['weight'], strict=True)
    print('Base network weights loaded successfully.')

    # Iterative pruning
    start = time.perf_counter()
    bayesian_pruning_log_dir = ''
    net = iterative_pruning_finetuning(model=net, bayesian_pruning_log_dir=bayesian_pruning_log_dir, num_iterations=50)
