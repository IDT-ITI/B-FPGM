...
def validation(net, criterion, optimizer, epoch: int, config, val_loader, sparsity: float = 0.0,
    ...
    # Save the model
    if save_model and (epoch_val_loss < min_loss or (epoch == config['epochs'] - 1) or (epoch % 10 == 0)):
        print(f'Saving state, epoch {epoch}')
        torch.save(net.state_dict(), f'{save_dir}/EXTD-{epoch}.pth')
...
def validation_for_optimization(net, criterion, val_loader) -> float:
    """ Calculates the validation losses to be used in the baysian optimisation """
    net.eval()
