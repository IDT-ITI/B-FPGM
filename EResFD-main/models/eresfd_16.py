...
class DetEncoder(nn.Module):
    def __init__(self, num_features=256):
        ...
        # self._modules = torch.jit.Attribute(OrderedDict(), OrderedDict)
        ...
        self.layer7 = CPM3_BN(int(self.num_features * CM_IN_FPN))
        self.layer8 = CPM3_BN(int(self.num_features * CM_IN_FPN))
        self.layer9 = CPM3_BN(int(self.num_features * CM_IN_FPN))
        self.layer10 = CPM3_BN(int(self.num_features * CM_IN_FPN))
        self.layer11 = CPM3_BN(int(self.num_features * CM_IN_FPN))
        self.layer12 = CPM3_BN(int(self.num_features * CM_IN_FPN))

        self._modules["b2_" + str(1)] = self.layer1
        self._modules["b2_" + str(2)] = self.layer2
        self._modules["b2_" + str(3)] = self.layer3
        self._modules["b2_" + str(4)] = self.layer4
        self._modules["b2_" + str(5)] = self.layer5
        self._modules["fpn"] = self.layer6
        self._modules["FEM_0"] = self.layer7
        self._modules["FEM_1"] = self.layer8
        self._modules["FEM_2"] = self.layer9
        self._modules["FEM_3"] = self.layer10
        self._modules["FEM_4"] = self.layer11
        self._modules["FEM_5"] = self.layer12
    ...
    def forward(self, inp) -> List[torch.Tensor]:
        ...
        # FPN
        conv0, conv1, conv2, conv3, conv4, conv5 = self.layer6(
            [conv0, conv1, conv2, conv3, conv4, conv5]
        )

        # FEM forward
        conv0 = self.layer7(conv0)
        conv1 = self.layer8(conv1)
        conv2 = self.layer9(conv2)
        conv3 = self.layer10(conv3)
        conv4 = self.layer11(conv4)
        conv5 = self.layer12(conv5)
...
class EResNet(nn.Module):
    def __init__(self, width_mult=1.0):
        ...
        self.m0 = DetEncoder(int(256 * width_mult))
        self.add_module("m0", self.m0)

        self._initialize_weights()

        # Auxilary modules conv weight init
        for i in range(3):
            self._auxmodules_initialize_weights(self.m0._modules["fpn"])
