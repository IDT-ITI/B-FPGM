...
class FullySepFPN(nn.Module):
    ...
    def _td_aggregate(self, lateral, upsample_target, weight):
    ...
    def forward(self, sources: List[torch.Tensor]):
        ...
        for idx, source in enumerate(sources):
            if idx in self.origin_idx:
                inp_org_blobs.append(source.clone())
        ...
        # For torch.script
        laterals = [self.laterals[0](sources[0]), self.laterals[1](sources[1]),
                    self.laterals[2](sources[2]), self.laterals[3](sources[3]),
                    self.laterals[4](sources[4])]
        ...
        for i in range(
            self.sep_idx, 0, -1
        ):  # NOTE: sep_idx==2:@torch.jit.script i={2,1}, sep_idx==3: i={3,2,1}, sep_idx==4: i={4,3,2,1}
            if i == self.sep_idx:
        ...
        # changed for torch.script
        l_td_paths[1] = self.itm[0](l_td_paths[1])
        l_td_paths[2] = self.itm[1](l_td_paths[2])
        l_td_paths[3] = self.itm[2](l_td_paths[3])
        ...
        # changed for torch.script
        # h_td_paths_copy = copy(h_td_paths)
        # h_td_paths2 = []
        # for idx, td in enumerate(h_td_paths):
        #     if idx + self.sep_idx in self.itm_idx[4 - 1 :]:
        #         temp = self._aggregate(
        #             self.itm[idx + 4 - 1](td),
        #             inp_org_blobs[idx + 4 - 1],
        #             w1[:, idx + 4 + 4],
        #         )
        #         h_td_paths2.append(temp)
        #     else:
        #         h_td_paths2.append(td)
