"""
B-FPGM
Hard-prune and fine-tune a model
CERTH 2024
"""
...
def main():
    """ Hard prune and finetune the model """
    seed = 0
    set_all_seeds(seed)
    ...
    wandb_config = {'epochs': args.epochs, 'lr_steps': args.schedule, 'batch_size': args.batch_size, 'pruner': 'FPGM',
                    'use_bayesian': True, 'bayesian_pruning_log_dir': '',
    ...
    save_dir = f'logs/Finetune/{run_name}'
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        train(train_loader, net, criterion, optimizer, epoch, current_learning_rate)
        torch.save(net.state_dict(), f'{save_dir}/ERes-Stop-{epoch}.pth')
...
def train(train_loader, model, criterion, optimizer, epoch, current_learning_rate):
    ...
    wandb.log({'Epoch': epoch, 'Train/Epoch_loss1': epoch_running_loss1, 'Train/Epoch_loss2': epoch_running_loss2,
               'Train/Loss': epoch_running_loss1 + epoch_running_loss2, 'Learning_rate': current_learning_rate})
