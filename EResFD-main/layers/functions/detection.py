...
class Detect(nn.Module):
    ...
    def __init__(self, cfg, log_file_path=None):
        ...
        self.avg_rejected = 0.0
        self.avg_survived = 0.0
    ...
    def forward(self, loc_data, conf_data, prior_data):
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):
                ...
                self.avg_survived += float(survived_boxes)
                self.avg_rejected += float(rejected_boxes)
                self.avg_rejection_rate += float(rejection_rate)
                self.counter += 1
                ...
                if self.top_k != -1:
                    ...
                    # for my_iterator in range(count):
                    #     # output[i, cl, my_iterator,:] = torch.cat(
                    #     #     (scores[ids[my_iterator]].view(-1,), boxes_[ids[my_iterator]]), 0
                    #     # )
                    #     temp = torch.cat(
                    #         (scores[ids[my_iterator]].view(-1,), boxes_[ids[my_iterator]]), 0
                    #     )
                    #     for my_iterator2 in range(5):
                    #         output[i, cl, my_iterator, my_iterator2] = temp[my_iterator2]
                    ...
                    for my_iterator in range(count):
                        ...
                        if my_iterator == 0:
                            result_3 = temp.view(1, -1).clone()
                        else:
                            result_3 = torch.cat((result_3, temp.view(1, -1)),dim=0) # noqa
                else:
                ...
                result_3_copy = result_3.clone() # noqa
                result_3_copy.zero_()
                result_2 = torch.cat((result_3_copy.unsqueeze(0), result_3.unsqueeze(0)), dim=0)
            if i == 0:
                result_1 = result_2.clone().unsqueeze(0) # noqa
            else:
                result_1 = torch.cat((result_2.clone().unsqueeze(0), result_1), dim=0) # noqa
        output = result_1 # noqa
        return output
